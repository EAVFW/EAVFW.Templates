...
This EAVFW project is set up to use Aspire. Aspire is an orchestrator for the entire application and will take care of configuring dependencies, building, and running the application. The resources that make up the application are defined in `apps/__EAVFW__.AppHost/AppHost.cs` including application code and external dependencies.

## EAVFW-Specific Notes

1. **Double-build requirement**: EAVFW projects require two build passes. The first build generates the manifest types from your model definitions. The second build compiles the application with those generated types. This is handled automatically by Aspire when you run the application.
2. **EAV model resources**: The EAV model handles database schema generation and migrations automatically. When you add entities or attributes to your manifest, the framework will update the database schema on startup.
3. **Main entry point**: Use `aspire run` to start the application. This will orchestrate all resources including the web app, database, and any other services defined in the AppHost.
4. **Manifest-driven development**: EAVFW applications are model-driven. Changes to entities, forms, and views are defined in the manifest files and reflected automatically in the running application.
